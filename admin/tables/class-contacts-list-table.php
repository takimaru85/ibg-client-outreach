...
final class Contacts_List_Table extends \WP_List_Table {
	...
	/**
	 * List service (list filter / bulk membership).
	 *
	 * @var List_Service
	 */
	...
	/**
	 * All lists keyed by id.
	 *
	 * @var array<int, Contact_List>
	 */
	private array $lists;
	 ...
	 */
	public function __construct( Contact_Repository $repository, List_Service $list_service, array $lists, Contacts_Page $page, bool $can_manage ) {
		parent::__construct(
			array(
				'singular' => 'contact',
				'plural'   => 'contacts',
	...
	/**
	 * The list/segment currently filtered on, if any.
	 *
	 * @return Contact_List|null
	 */
	public function get_active_list(): ?Contact_List {
		$filters = $this->get_filter_args();
		$id      = (int) ( $filters['list_id'] ?? 0 );
		return $id > 0 ? ( $this->lists[ $id ] ?? null ) : null;
	}
	...
	protected function get_bulk_actions(): array {
		...
		if ( $this->has_static_lists() ) {
			$actions['add_to_list']      = __( 'Add to list →', 'ibg-client-outreach' );
			$actions['remove_from_list'] = __( 'Remove from list →', 'ibg-client-outreach' );
		}
	...
	/**
	 * Whether any static lists exist.
	 *
	 * @return bool
	 */
	private function has_static_lists(): bool {
	...
	/**
	 * Render the target-list selector next to the bulk action dropdown.
	 *
	...
	protected function bulk_actions( $which = '' ): void {
		...
		if ( 'top' !== $which || ! $this->can_manage || ! $this->has_static_lists() ) {
			return;
		}
		?>
		<label class="screen-reader-text" for="ibg-list-target"><?php esc_html_e( 'Target list for add/remove', 'ibg-client-outreach' ); ?></label>
		<select name="ibg_list_target" id="ibg-list-target">
			<option value="0"><?php esc_html_e( '→ choose list', 'ibg-client-outreach' ); ?></option>
			<?php foreach ( $this->lists as $list ) : ?>
				<?php if ( $list->is_segment() ) { continue; } ?>
	...
	protected function extra_tablenav( $which ): void {
		...
		<div class="alignleft actions ibg-filters">
			<?php if ( ! empty( $this->lists ) ) : ?>
				<label class="screen-reader-text" for="ibg-filter-list"><?php esc_html_e( 'Filter by list or segment', 'ibg-client-outreach' ); ?></label>
				<select name="list_id" id="ibg-filter-list">
					<option value=""><?php esc_html_e( 'All lists & segments', 'ibg-client-outreach' ); ?></option>
					<?php foreach ( array( Contact_List::TYPE_STATIC => __( 'Lists', 'ibg-client-outreach' ), Contact_List::TYPE_SEGMENT => __( 'Segments', 'ibg-client-outreach' ) ) as $type => $group ) : ?>
						<optgroup label="<?php echo esc_attr( $group ); ?>">
							<?php foreach ( $this->lists as $list ) : ?>
								<?php if ( $list->type !== $type ) { continue; } ?>
								<option value="<?php echo (int) $list->id; ?>" <?php selected( (int) ( $filters['list_id'] ?? 0 ), $list->id ); ?>><?php echo esc_html( $list->name ); ?></option>
							<?php endforeach; ?>
						</optgroup>
					<?php endforeach; ?>
				</select>
	...
	private function to_query_args( array $filters ): array {
		...
		// A list/segment filter supplies base conditions; explicit filters narrow it further.
		$list = ! empty( $filters['list_id'] ) ? ( $this->lists[ (int) $filters['list_id'] ] ?? null ) : null;
		if ( $list ) {
