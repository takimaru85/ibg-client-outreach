...
( function () {
	...
	function showFile() {
		...
		if ( input.files && input.files.length ) {
			fileName.textContent = input.files[ 0 ].name + ' (' + Math.round( input.files[ 0 ].size / 1024 ) + ' KB)';
	...
	function render( p ) {
		var pct = total ? Math.min( 100, Math.round( 100 * p.processed / total ) ) : 100;
		bar.style.width = pct + '%';
		status.textContent = sprintf( cfg.i18n.importing, p.processed, total );

		[ 'created', 'updated', 'skipped_invalid', 'skipped_existing', 'preserved', 'failed', 'listed' ].forEach( function ( key ) {
